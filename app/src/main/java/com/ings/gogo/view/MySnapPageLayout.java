...
public class MySnapPageLayout extends ViewGroup {
	...
	public interface McoySnapPage {
		...
		/**
		 * 是否滑动到最顶端 第二页必须自己实现此方法，来判断是否已经滑动到第二页的顶部 并决定是否要继续滑动到第一页
		 */
		boolean isAtTop();

		/**
		 * 是否滑动到最底部 第一页必须自己实现此方法，来判断是否已经滑动到第二页的底部 并决定是否要继续滑动到第二页
		 */
		boolean isAtBottom();
	}
	...
	/**
	 * @mcoy add computeScroll方法会调用postInvalidate()方法， 而postInvalidate()方法中系统
