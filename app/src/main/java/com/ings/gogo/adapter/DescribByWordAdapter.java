...
public class DescribByWordAdapter extends BaseAdapter {
	private Context sContext;
	private List<String> dataList;

	public DescribByWordAdapter(Context context, List<String> strings) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(sContext).inflate(
					R.layout.layout_detaildescbyworditem, null);
			viewHolder = new ViewHolder();
			viewHolder.mDescribByWordTextView = (TextView) convertView
					.findViewById(R.id.descriGoodsByWordTv);
			convertView.setTag(viewHolder);
		} else {
			viewHolder = (ViewHolder) convertView.getTag();
		}
		viewHolder.mDescribByWordTextView.setText(dataList.get(position));
		return convertView;
	}

	static class ViewHolder {
		TextView mDescribByWordTextView;
