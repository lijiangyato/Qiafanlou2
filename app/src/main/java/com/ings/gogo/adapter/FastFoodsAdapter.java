...
public class FastFoodsAdapter extends BaseAdapter {
	private Context context;
	private LayoutInflater layoutInflater;
	private List<FastFoodsEntity.FastFoodsDatas> entitys;

	public FastFoodsAdapter(Context context, List<FastFoodsDatas> entitys) {
		super();
	...
	private void initializeViews(FastFoodsEntity.FastFoodsDatas entity,
		...
		if (entity.getStock() == 0) {
			holder.mFastFoodsIsOut.setVisibility(View.VISIBLE);
			holder.mFastFoodsIsOut.setBackgroundResource(R.drawable.isno);
		} else {
			holder.mFastFoodsIsOut.setVisibility(View.GONE);
		}
