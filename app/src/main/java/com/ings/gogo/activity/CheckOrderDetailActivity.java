...
import com.ings.gogo.adapter.DetailOrderGoodsAdapter;
import com.ings.gogo.application.UILApplication;
import com.ings.gogo.base.BaseData;
import com.ings.gogo.entity.DetailOrderEntity;
...
public class CheckOrderDetailActivity extends BaseActivity implements
	...
	Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case GET_DATA_OK:
				mOrderTime.setText("下单时间："
						+ mDetailEntity.getData().getCreatedate());
				mOrderOrderID.setText("订单编号："
						+ mDetailEntity.getData().getOrderno());
				mOrderUserName.setText(mDetailEntity.getData().getConsignee()
						.getConsignee_name()
						+ "      "
						+ mDetailEntity.getData().getConsignee()
								.getConsignee_phone());
				mOrderUserAddress.setText(mDetailEntity.getData()
						.getConsignee().getConsignee_add());
				mOrderPayWay.setText("支付方式："
						+ mDetailEntity.getData().getPayway());
				mOrderTotalMoney.setText("商品合计：" + "¥"
						+ mDetailEntity.getData().getTotal());
				mOrderPostMoney.setText("运        费：" + "¥"
						+ mDetailEntity.getData().getPostage());
				if (mDetailEntity.getData().getState().equals("已作废")) {
					mOrderState.setText("超时作废");
				} else {
					mOrderState.setText(mDetailEntity.getData().getState());
				}

				mGoodsAdapter = new DetailOrderGoodsAdapter(
						getApplicationContext(), mDetailEntity.getData()
								.getOrderPros());
	...
	protected void getDetailData() {
		...
		call.enqueue(new Callback() {
			...
			public void onResponse(final Response response) throws IOException {
				...
				Gson gson = new Gson();
				mDetailEntity = gson.fromJson(orderDetailBody,
