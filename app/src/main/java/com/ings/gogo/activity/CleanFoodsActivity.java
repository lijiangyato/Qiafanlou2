...
public class CleanFoodsActivity extends BaseActivity {
	private static final int GET_DATA_OK = 1;
	private static final int GET_PICOK = 0;
	private static final String isToday = "2";
	private ListView mCleanFoodsLV;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.layout_cleanfoods);
		...
		LogUtils.e("今日净菜的相关值----》》》", pointID + pointName + rangers);
	...
	private void ininitViews() {
		// TODO Auto-generated method stub
		mCleanFoodsLV = (ListView) this.findViewById(R.id.listView_cleanFoods);
		...
		mGoodsDescTv = (TextView) viewpagerTop.findViewById(R.id.alltop_title);
		...
		mCleanFoodsLV.addHeaderView(viewpagerTop);
		mCleanFoodsLV.addFooterView(viewpagerBottom);
		...
		public void handleMessage(android.os.Message msg) {
			...
			case GET_DATA_OK:
				if (mFastEntity.getData().size() == 0) {
					mGoodsDescTv.setText("");
				} else {
					mGoodsDescTv.setText("今日净菜（共"
							+ mFastEntity.getData().size() + "个商品）");
				}
				mCleanFoodsLV.setAdapter(mFastAdapter);
				mCleanFoodsLV.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						Intent intent = new Intent(getApplicationContext(),
								DetailCleanFoodsActivity.class);
						Bundle bundle = new Bundle();
						bundle.putString("cleanFoodsName", mFastEntity
								.getData().get((int) id).getProname());
						bundle.putString("cleanFoodsPrice", mFastEntity
								.getData().get((int) id).getPrice()
								+ "");
						bundle.putString("cleanFoodsImage", mFastEntity
								.getData().get((int) id).getImgurl());
						bundle.putString("proid",
								mFastEntity.getData().get((int) id).getProid());
						bundle.putString("isToday", isToday);
						bundle.putInt("Stock",
								mFastEntity.getData().get((int) id).getStock());
	...
	protected void getProductDatas() {
		...
		final Request request = new Request.Builder().url(
				BaseData.GET_FAST_PRODUCTS + pointID
						+ "&categoryid=8792CBB4-B0CA-4276-AC5A-1F1B08562926"
		...
		call.enqueue(new Callback() {
			...
			public void onResponse(final Response response) throws IOException {
				...
				LogUtils.e("净菜body--->>", getProductBody);
				Gson gson = new Gson();
