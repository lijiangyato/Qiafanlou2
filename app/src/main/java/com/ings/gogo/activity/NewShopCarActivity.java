...
public class NewShopCarActivity extends BaseActivity implements OnClickListener {
	...
	// // TODO Auto-generated method stub
	// LogUtils.e("shopcar执行到了没", "shopcar执行到了没");
	// if (mListAdapter.getCount() == 0) {
	// LogUtils.e("暂不刷新", "暂不刷新");
	// } else {
	// mListAdapter.notifyDataSetChanged();
	// }
	//
	...
	private class ListAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			if (view == null) {
				...
				view = LayoutInflater.from(NewShopCarActivity.this).inflate(
	...
	protected void onPause() {
		// TODO Auto-generated method stub
		super.onPause();
		LogUtils.e("新的的pause 执行到了", "新的购物车中的pause 执行到了");
		mSeclectedEntity.clear();
	}
		...
		case R.id.tv_cart_buy_or_del:
			if (isBatchModel) {
				...
				if (totalPrice != 0) {
					//
					if (mSeclectedEntity.size() == 0) {
						Toast.makeText(NewShopCarActivity.this, "请重新选择要支付的商品",
								Toast.LENGTH_SHORT).show();
					...
					Toast.makeText(NewShopCarActivity.this, "请选择要支付的商品",
							Toast.LENGTH_SHORT).show();
					// mListAdapter.notifyDataSetChanged();
					return;
				}
	...
	public void invisibleOnScreen() {
		...
		if (entity.size() == 0) {
			...
			mListAdapter = new ListAdapter();
			int fuck = mListAdapter.getCount();
			LogUtils.e("暂时没有数据dd", "暂时没有数据" + fuck);
			mListView.setAdapter(mListAdapter);
			mListAdapter.notifyDataSetChanged();
		}
