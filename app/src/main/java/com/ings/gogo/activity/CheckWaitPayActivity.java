...
public class CheckWaitPayActivity extends BaseActivity implements
		OnClickListener {
		...
		public void handleMessage(Message msg) {
			...
			case CANCEL_OK:
				...
				if (mResultEntity.getSuccess().equals(true)) {
					new MyCheckAdapter().notifyDataSetChanged();
					getOrderData();
				}
	...
	protected void getOrderData() {
		...
		final Request request = new Request.Builder()
				...
				.url(BaseData.GET_DIFFERENT_STATE + "0").build();
		// new call
		...
		call.enqueue(new Callback() {
			...
			public void onResponse(final Response response) throws IOException {
				...
				LogUtils.e("待付款订单列表body--->>", orderListBody);
				Gson gson = new Gson();
	...
	private class MyCheckAdapter extends BaseAdapter {
		...
		private void initializeViews(
			...
			} else if (entity.getState().equals("待付款")
					|| entity.getState().equals("待配送")) {
